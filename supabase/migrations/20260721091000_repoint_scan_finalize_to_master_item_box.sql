-- Phase 5/6 RPCs were written against the old fused box_definitions table.
-- Repoint them at the split boxes / master_item_boxes / box_layer_requirements
-- shape from 20260721090000_restructure_box_master_data.sql. Behavior is
-- unchanged; only the underlying joins move.
...
drop function if exists public.finalize_packing_session(uuid, uuid);
...
revoke execute on function public.start_packing_session(uuid, uuid, uuid) from public, anon;
...
grant execute on function public.start_packing_session(uuid, uuid, uuid) to authenticated;
