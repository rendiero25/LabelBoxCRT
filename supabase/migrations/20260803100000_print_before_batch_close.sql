-- Cetak label sebelum batch ditutup.
--
-- Alur lapangan: operator scan seluruh produk, mencetak label, menempelkannya,
-- lalu menutup batch sebagai penyimpanan akhir. Aturan lama memaksa urutan
-- terbalik (tutup dulu, baru boleh cetak), sehingga batch sudah terkunci
-- sebelum operator tahu labelnya berhasil keluar dari printer.
--
-- Syaratnya tidak dihapus, hanya dipindah: batch yang belum ditutup boleh
-- dicetak setelah setiap produk yang diminta box batch punya scan diterima,
-- yaitu syarat yang sama dengan close_label_box_batch. Batch yang sudah
-- ditutup tetap boleh dicetak ulang apa adanya, karena label bisa rusak,
-- hilang, atau kehabisan kertas setelah batch disimpan.
--
-- print_jobs.packing_session_id wajib terisi, sedangkan box yang belum pernah
-- discan belum punya sesi. Backfill yang dulu hanya ada di close dilakukan di
-- sini juga, kalau tidak label box tersebut tidak akan pernah ikut tercetak.
