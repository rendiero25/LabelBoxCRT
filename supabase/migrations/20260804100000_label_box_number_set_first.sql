-- Nomor box label dibaca operator sebagai "set ke sekian, box ke sekian",
-- dan label ditempel berurutan per set. Format lama menaruh nomor box lebih
-- dulu ('B' + box + set dua digit), sehingga satu set berjalan B101, B201,
-- B301 lalu set berikutnya B102 — urutan yang melompat-lompat di tangan
-- operator.
--
-- Format baru: 'B' + nomor set + nomor box dua digit, jadi satu set terbaca
-- runtut B101, B102, B103, lalu set kedua B201, B202, B203.
--
...
begin
  ...
  select
    ...
    box.box_no,
    series.set_no,
    'B' || series.set_no::text || lpad(box.box_no::text, 2, '0'),
    concat_ws(
      '|',
