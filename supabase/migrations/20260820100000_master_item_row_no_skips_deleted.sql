-- Nomor urut Master Item pada label dihitung tanpa baris yang sudah terhapus.
--
...
-- riwayat dan ditandai deleted_at. View public.master_item_row_numbers -- yang
-- dibaca layar admin -- sudah ikut menyaring baris itu, tetapi
-- create_label_box_batch dan rebuild_label_box_batch masih memberi nomor
-- dengan row_number() atas seluruh isi public.master_items.
--
-- Akibatnya angka yang tercetak di label dan tertanam di QR bisa lebih besar
-- daripada angka yang dilihat admin saat memilih Master Item-nya: setiap baris
-- terhapus yang item_code-nya berada lebih awal menggeser nomor semua Master
-- Item di bawahnya. Aturannya satu, bukan dua: nomor urut adalah posisi Master
-- Item di antara Master Item yang belum terhapus, diurutkan menurut item_code.
--
-- Batch yang sudah ada tidak disentuh. Nomornya tersimpan di
-- label_box_batches.master_item_row_no dan ikut tercetak di QR yang sudah
-- menempel di box; menulis ulang angkanya justru membuat label yang beredar
-- tidak lagi cocok dengan datanya. Yang berubah hanya nomor untuk batch
-- berikutnya.
--
-- create_label_box_batch sekalian menolak Master Item yang sudah terhapus,
-- persis seperti rebuild_label_box_batch. Tanpa itu, baris terhapus yang
-- is_active-nya dinyalakan kembali lewat set_master_item_active akan lolos
-- pemeriksaan lalu gagal di tengah jalan dengan galat not-null, karena ia tidak
-- punya nomor urut sama sekali.

create or replace function public.create_label_box_batch(
  p_supplier_id uuid,
...
-- Penanda untuk perubahan berikutnya: definisi nomor urut ada di tiga tempat
-- dan ketiganya harus berubah bersama-sama.
comment on view public.master_item_row_numbers is
  'Nomor urut Master Item: posisi di antara Master Item yang belum terhapus, diurutkan item_code. Aturan yang sama ditulis ulang di create_label_box_batch dan rebuild_label_box_batch -- ubah ketiganya bersama-sama.';
