-- Master Item yang sudah punya riwayat boleh dihapus; riwayatnya tetap utuh.
--
-- Menolak penghapusan hanya karena ada kiriman lama memaksa admin menyimpan
-- baris yang tidak dipakai lagi selamanya. Sebaliknya, menghapus barisnya
-- betul-betul akan memutus label_box_batches dan packing_sessions yang
-- menunjuknya -- dan seluruh jalur scan, verifikasi, serta cetak ulang membaca
-- master_item_id itu.
--
-- Jalan tengahnya: baris Master Item tetap ada sebagai jangkar riwayat, tetapi
-- ditandai terhapus dan hilang dari seluruh daftar, pilihan, dan penomoran.
-- Bagi pemakainya ia terhapus; bagi riwayatnya tidak ada yang berubah, dan
-- label lama tetap memakai salinan datanya sendiri.
--
-- Master Item yang belum pernah dipakai kirim tetap dihapus betulan, bersama
-- Box Definition dan product mapping miliknya.
--
-- Kode item dan Part No milik Master Item terhapus dilepas kembali: indeks
-- uniknya kini hanya berlaku untuk baris yang belum terhapus, jadi Part No
-- yang sama bisa didaftarkan ulang tanpa bentrok dengan baris yang tak
...
drop index if exists public.master_items_item_code_key;

create unique index master_items_item_code_key
  on public.master_items (lower(btrim(item_code)))
  where deleted_at is null;

drop index if exists public.master_items_part_no_key;

create unique index master_items_part_no_key
  on public.master_items (upper(btrim(part_no)))
  where deleted_at is null;

create index if not exists master_items_deleted_at_idx
  on public.master_items (deleted_at);

-- Nomor urut dihitung dari Master Item yang masih hidup saja: yang terhapus
-- sudah tidak muncul di daftar mana pun, jadi ia tidak boleh ikut memakan
-- nomor.
create or replace view public.master_item_row_numbers
with (security_invoker = on) as
select
  item.id as master_item_id,
  (row_number() over (order by item.item_code))::integer as row_no
