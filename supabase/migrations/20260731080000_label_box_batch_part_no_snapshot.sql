-- Tabel data label box di halaman scan menampilkan kolom Master Item.
-- Sampai sekarang isinya item_code, yaitu kode internal hasil autogen
-- (dm-0001) yang tidak dikenali operator di lapangan. Yang dipakai operator
-- adalah part_no, kode yang sama dengan yang tercetak di label dan di QR.
--
-- Part No disnapshot ke batch mengikuti pola snapshot lain di tabel ini
-- (lihat 20260728120000): label_box_batches_select membuka semua batch untuk
-- app user, sedangkan master_items_select hanya membuka master item aktif,
-- jadi tanpa snapshot baris batch lama kehilangan Part No begitu master
-- itemnya dinonaktifkan.

alter table public.label_box_batches
  add column part_no_snapshot text;

update public.label_box_batches batch
set part_no_snapshot = item.part_no
from public.master_items item
where item.id = batch.master_item_id;

alter table public.label_box_batches
  alter column part_no_snapshot set not null,
  add constraint label_box_batches_part_no_snapshot_check
    check (btrim(part_no_snapshot) <> '');
