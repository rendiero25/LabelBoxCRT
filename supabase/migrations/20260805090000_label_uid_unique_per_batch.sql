-- Keunikan label discan dipagari per batch label box, bukan global.
--
-- Aturan lama menyimpan satu indeks unik pada label_uid untuk seluruh tabel,
-- selamanya. Akibatnya QR produk yang sama ditolak LABEL_ALREADY_SCANNED pada
-- batch yang sama sekali berbeda: DN lain, lot lain, Master Item lain. Yang
-- ingin dijaga sebenarnya lebih sempit, yaitu satu keping tidak masuk dua box
-- dalam satu kiriman.
--
-- label_box_batch_id disimpan di barisnya sendiri karena batch hanya bisa
-- dicapai lewat label_boxes, dan indeks parsial tidak bisa menempuh join.
-- Kolomnya nullable: sesi packing yang tidak berasal dari batch label box
-- (jalur lama) tetap sah, dan bagi baris seperti itu penjagaan tetap berlaku
-- antar sesama baris tanpa batch lewat IS NOT DISTINCT FROM.

alter table public.packing_session_scans
  add column if not exists label_box_batch_id uuid references public.label_box_batches(id) on delete restrict;

update public.packing_session_scans scan
set label_box_batch_id = box.batch_id
from public.label_boxes box
where box.packing_session_id = scan.packing_session_id
  and scan.label_box_batch_id is null;

create index if not exists packing_session_scans_batch_idx
  on public.packing_session_scans (label_box_batch_id);

drop index if exists public.packing_session_scans_accepted_label_uid_idx;

create unique index packing_session_scans_accepted_label_uid_per_batch_idx
  on public.packing_session_scans (label_box_batch_id, label_uid)
