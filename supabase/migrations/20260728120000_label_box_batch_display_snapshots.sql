-- Snapshot the delivery/supplier/item display fields onto
-- label_box_batches, following the same pattern already used on
-- public.print_jobs. label_box_batches_select lets any active operator
-- read every batch, but suppliers_select/master_items_select/
-- delivery_numbers_select only expose rows tied to active suppliers,
-- active master items, and active delivery numbers. Without a snapshot,
-- closing a Delivery Number or deactivating a supplier/master item
-- silently hides an already-generated batch from the operator table even
-- though the foreign keys (on delete restrict) guarantee the row still
-- exists.

alter table public.label_box_batches
  add column supplier_code_snapshot text,
  add column item_code_snapshot text,
  add column delivery_number_snapshot text,
...
set
  ...
  delivery_number_snapshot = dn.delivery_number,
  delivery_date_snapshot = dn.delivery_date
from public.suppliers supplier, public.master_items item, public.delivery_numbers dn
where supplier.id = batch.supplier_id
  and item.id = batch.master_item_id
  and dn.id = batch.delivery_number_id;

alter table public.label_box_batches
  alter column supplier_code_snapshot set not null,
  alter column item_code_snapshot set not null,
  alter column delivery_number_snapshot set not null,
  alter column delivery_date_snapshot set not null,
  add constraint label_box_batches_supplier_code_snapshot_check
    check (btrim(supplier_code_snapshot) <> ''),
  add constraint label_box_batches_item_code_snapshot_check
    check (btrim(item_code_snapshot) <> ''),
  add constraint label_box_batches_delivery_number_snapshot_check
    check (btrim(delivery_number_snapshot) <> '');
