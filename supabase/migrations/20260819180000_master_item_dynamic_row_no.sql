-- Nomor urut Master Item kembali dinamis.
--
-- Nomor tetap per Master Item dicabut atas permintaan pemakainya: nomor yang
-- dipakai adalah posisi Master Item di dalam daftar seluruh Master Item yang
-- diurutkan menurut item_code, dihitung saat batch label box dibuat. Batch
-- yang sudah ada tetap membawa nomor yang tersimpan di
-- label_box_batches.master_item_row_no, jadi QR yang sudah tercetak tidak
-- berubah -- yang bergeser hanya nomor untuk batch berikutnya.
--
-- Konsekuensinya diketahui dan diterima: menambah atau menghapus satu Master
-- Item menggeser nomor Master Item lain, sehingga part yang sama bisa membawa
-- nomor berbeda di dua kiriman.

drop trigger if exists master_items_assign_row_no on public.master_items;

drop function if exists private.assign_master_item_row_no();

alter table public.master_items
  drop column if exists row_no;

-- Nomor yang sama dengan yang akan dipakai QR, supaya daftar Master Item di
-- layar tidak menghitungnya sendiri dengan aturan urutan yang bisa berbeda
-- dari milik Postgres.
...
grant select on table public.master_item_row_numbers to authenticated;
